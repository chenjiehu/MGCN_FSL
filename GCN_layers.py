...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, init, N_way = 5):
        super(GCN, self).__init__()
        self.N_way = N_way
        self.sigama1 = nn.Parameter(torch.Tensor(1), requires_grad=True)
        self.sigama2 = nn.Parameter(torch.Tensor(1), requires_grad=True)
        self.sigama1.data.fill_(30)
        self.sigama2.data.fill_(30)
    ...
    def forward(self, x):
        adj = self.AdjacencyCompute(sigama=self.sigama1, features=x)
        x = F.dropout(F.relu(self.bn1(self.gc1(x, adj))), self.dropout, training=self.training)
        adj = self.AdjacencyCompute(sigama=self.sigama2, features=x)
        x = F.relu(self.gc2(x, adj))
        return x
        #return F.log_softmax(x, dim=1)
    def AdjacencyCompute(self, sigama, features):
        # features = F.normalize(features, p=2, dim=1)
        N = features.size(0)
        temp = torch.norm(features.repeat(N, 1) - features.repeat(1, N).view(N * N, -1), dim=1)
        # sigama
        adjacency = torch.exp(-temp.pow(2) / sigama).view(N, N)
        temp, position = torch.topk(adjacency, round(N / self.N_way), dim=1, sorted=False, out=None)
        adjacency0 = torch.zeros(N, N).cuda()
        for num in range(N):
            adjacency0[num, position[num]] = 1
        adjacency = torch.mul(adjacency, adjacency0)
        return adjacency